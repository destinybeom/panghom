...
def Mtr_Read(request, entry_id=None):
    ...
    try:
        usingCnt = len(models.GayaMaterialSN.objects.get(msn_mtr=current_entry, msn_using=1))
    except:
        usingCnt = 0
    try:
        outCnt = len(models.GayaMaterialSN.objects.get(msn_mtr=current_entry, mtr_out=1))
    except:
        outCnt = 0
    ...
    remnants = int(current_entry.mtr_total) - int(usingCnt) - int(outCnt)
...
def HA201TP_Add(request):
    ...
    new_entry = models.HA201_TP(pro_name='HA201_TP', pro_pn=pn, pro_sn=sn, pro_emp=emp,
...
def Msn_Add(request):
    ...
    if request.POST.has_key('msn_sn') == False:
        ...
        if len(request.POST['msn_sn']) == 0:
            ...
            snList = chk.findall(request.POST['msn_sn'])
    ...
    for sn in snList:
        new_entry = models.GayaMaterialSN(msn_mtr=mtr, msn_class=mtr.mtr_class, msn_emp=emp, msn_pn=pn, msn_sn=sn,
                msn_state=state, msn_author=author, msn_note=note)
        try:
            new_entry.save()
        except:
